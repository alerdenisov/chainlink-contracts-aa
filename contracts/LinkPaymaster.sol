...
/**
 * A LINK-based paymaster that accepts token deposits
 ...
 */
contract LinkPaymaster is BasePaymaster {
    ...
    constructor(IEntryPoint _entryPoint, AggregatorV3Interface _priceFeed, IERC20 _linkToken) BasePaymaster(_entryPoint) {
     ...
     */
    function addDeposit(address account, uint256 amount) external {
    ...
    function withdrawTokensTo(address target, uint256 amount) public {
        ...
        linkToken.safeTransfer(target, amount);
    }
    ...
    function getTokenValueOfEth(uint256 ethBought) internal view virtual returns (uint256 requiredTokens) {
        ...
        return ethBought * uint256(answer) / (1 ** 18);
    }
    ...
    internal view override returns (bytes memory context, uint256 validationData) {
        ...
        return (abi.encode(account, linkToken, gasPriceUserOp, maxTokenCost, maxCost),0);
    }
    ...
    function _postOp(PostOpMode mode, bytes calldata context, uint256 actualGasCost) internal override {
        ...
        if (mode != PostOpMode.postOpReverted) {
            ...
            linkToken.safeTransferFrom(account, address(this), actualTokenCost);
        } else {
